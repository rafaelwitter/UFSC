import sqlite3
from controladores.DAO import DAO
class PedidoDB(DAO):
	def __init__(self, db):
		self.conn = sqlite3.connect(db)
		self.cur = self.conn.cursor()
		self.cur.execute(
			"CREATE TABLE IF NOT EXISTS pedidos_excluidos (codigo INTEGER PRIMARY KEY, nome TEXT, cpf INTEGER,telefone TEXT, rua TEXT, numero INTEGER, complemento TEXT, pedido TEXT, data TEXT, status TEXT)"			
			)
		self.cur.execute(
	...
	def fetch_excluidos(self):
		self.cur.execute("SELECT * FROM pedidos_excluidos \n")
		rows = self.cur.fetchall()
		return rows
	...
	def insert_excluido(self, codigo, nome, cpf, telefone, rua, numero, complemento, pedido, status, data):
		self.cur.execute("INSERT INTO pedidos_excluidos VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		(codigo, nome, cpf, telefone, rua, numero, complemento, pedido, status, data)
		)
		self.conn.commit()
