...
class ControladorCliente():
    def __init__(self, controlador_principal):
        ...
        self.__cliente_dao = ClienteDB('./controladores/ControladorCliente/cliente.db')
        self.__main_view = TelaCliente()
        self.__controlador_principal = controlador_principal
        self.religa_sistema()

    def religa_sistema(self):
        clientes = self.__cliente_dao.fetch()
        fidelidade = 0
    ...
    def altera_senha(self):
        ...
        for cliente in self.__clientes:
            if cliente == self.__cliente_atual and eventos == 'Alterar':
                ...
                self.__cliente_dao.update(cliente.cpf, cliente.nome, cliente.telefone,
    ...
    def altera_nome(self):
        ...
        for cliente in self.__clientes:
            if cliente == self.__cliente_atual and eventos == 'Alterar':
                ...
                self.__cliente_dao.update(cliente.cpf, dados[0], cliente.telefone,
        ...
        try:
            if isinstance(novo_cliente, Cliente):
                if self.existe_cliente(novo_cliente):
                    ...
                    self.__cliente_dao.insert(novo_cliente.cpf, novo_cliente.nome, novo_cliente.telefone,
    ...
    def ve_clientes(self):
        if self.__clientes:
            self.__main_view.open_clientes(self.__clientes)
        else:
    ...
    def ve_clientes_excluidos(self):
        if self.__clientes_excluido:
            self.__main_view.open_clientes(self.__clientes_excluido)
        else:
    ...
    def ve_dados(self):
        self.__main_view.show_msg("Dados", str(self.__cliente_atual))
